...
class ProtoRouter(object):
    ...
    def process_incoming_nat(self, event, ip_pkt, transport_pkt):
        if ip_pkt.dstip != PUBLIC_IP:
            log_color(
                RED,
                f"DROP: Paquete de internet dirigido a IP inválida ({ip_pkt.dstip})",
            )
            return
        ...
        if trans:
            ...
            log_color(
                CYAN,
                f"NAT ENTRANTE [NUEVO]: {PUBLIC_IP}:{trans['nat_port']} -> {trans['private_ip']}:{trans['private_port']}",
            )
        else:
            log_color(
                RED,
                f"DROP: No hay traducción activa para puerto {transport_pkt.dstport}",
            )
    ...
    def process_outgoing_nat(self, event, packet, ip_pkt, transport_pkt):
        ...
        if not trans:
            log_color(RED, "DROP: Puertos NAT agotados")
            return
        ...
        if remote_mac is None:
            ...
            log_color(
                YELLOW, f"PAQUETE EN ESPERA: Resolviendo ARP para {trans['remote_ip']}"
            )
            return
        ...
        log_color(
            GREEN,
            f"NAT SALIENTE [NUEVO]: {trans['private_ip']}:{trans['private_port']} -> {PUBLIC_IP}:{trans['nat_port']}",
        )
    ...
    def handle_arp(self, event, packet):
        ...
        if arp_pkt.opcode == arp.REQUEST and arp_pkt.protodst in (
            ...
            log_color(
                GREEN, f"ARP APRENDIDO: Servidor {target_ip} está en MAC {target_mac}"
            )
    ...
    def _handle_flow_removal(self, event):
        ...
        if trans:
            log_color(
                YELLOW,
                f"TIMEOUT: Conexión {trans['private_ip']}:{trans['private_port']} expiró.",
            )
